...
class BatchPredictor:
    ...
    def predict_batch(
        ...
        for i in range(0, len(X), batch_size):
            ...
            try:
                ...
                if self.preprocessor:
                    batch_processed = self.preprocessor.preprocess_health_data(batch, fit=False)
                else:
        ...
        logger.info(f"Batch prediction complete: {len(predictions)}/{len(X)} successful")

        return result

    def predict_from_csv(self, csv_path: Path, batch_size: int = 32) -> dict[str, Any]:
    ...
    def save_predictions(self, predictions: dict[str, Any], output_path: Path) -> None:
